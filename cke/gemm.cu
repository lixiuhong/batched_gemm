...
int  main (int argc, char** argv) {
	...
	cudaStream_t *stream;
	stream = (cudaStream_t*) malloc(BATCH * sizeof(cudaStream_t));

	for (int i=0; i<BATCH; ++i)
		ErrChk(cudaStreamCreate(&stream[i]));
	...
	//warm-up
	for (int i=0; i<BATCH; ++i){
		ErrChk(cublasSetStream(handle, stream[i]));
	...
	for (int i=0; i<BATCH; ++i){
		...
		ErrChk(cudaFree(B[i]));		
		ErrChk(cudaFree(C[i]));		
		ErrChk(cudaStreamDestroy(stream[i]));
	}
